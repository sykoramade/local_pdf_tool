    ...
    >
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 24px', width: '100%' }}>
            ...
            {TOOL_ORDER.map((key, idx) => {
              ...
              return (
                <div
                  ...
                  onClick={() => { if (!isSoonTool) setActive(key) }}
                  style={{
                    flex: 1,
                    position: 'relative',
                    zIndex: 1,
                    ...
                    gap: 5,
                    padding: '7px 8px',
                    cursor: isSoonTool ? 'default' : 'pointer',
                    fontSize: 10,
                    fontWeight: 700,
                    letterSpacing: '.07em',
        ...
        <div
          style={{
            width: '100%',
            border: `1.5px solid ${isSoon ? 'rgba(255,255,255,.08)' : (tool as ActiveTool).accentColor}`,
            borderRadius: 12,
        ...
        >
          ...
          <div style={{
            ...
            <div style={{
              ...
              flexShrink: 0,
              background: isSoon ? 'rgba(255,255,255,.07)' : (tool as ActiveTool).iconBg,
            }}>
              {tool.icon}
            </div>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', lineHeight: 1.4, marginTop: 2 }}>
                {isSoon ? 'Not available yet' : (tool as ActiveTool).desc}
              </div>
            </div>
          </div>

          {/* Drop target — active tool */}
          {!isSoon && (
            <div style={{
              display: 'flex',
              ...
              <svg style={{ marginBottom: 4, opacity: .22 }} width="44" height="52" viewBox="0 0 44 52" fill="none">
              ...
              <p style={{ fontSize: 14, fontWeight: 500, color: 'rgba(255,255,255,.5)' }}>
                {(tool as ActiveTool).dropLabel}
              </p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,.26)' }}>
                {(tool as ActiveTool).dropSub}
              </p>
              <Link
                href={(tool as ActiveTool).href}
                style={{
                  marginTop: 6,
                  padding: '9px 28px',
                  borderRadius: 7,
                  background: hoveredBtn ? (tool as ActiveTool).btnHover : (tool as ActiveTool).btnColor,
                  color: 'white',
                  fontSize: 12,
                  fontWeight: 600,
                  letterSpacing: '.02em',
                  ...
                  boxShadow: hoveredBtn ? '0 4px 16px rgba(99,102,241,.35)' : 'none',
                }}
